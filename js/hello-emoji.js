...
(function($) {
	...
	$(function() {
		...
		$('textarea#content').textcomplete(strategy);
		$('textarea#comment').textcomplete(strategy);

		$('.wpcollab-hello-emoji').helloEmoji();
	...
	$.fn.helloEmoji = function() {
		var keys = '\\' + emojis.join('|');

		return this.each(function() {
			var regex = new RegExp(':(' + keys + '):', 'g');
			$(this).html($(this).html().replace(regex, $.fn.helloEmoji.replace));
		});
	};

	$.fn.helloEmoji.replace = function() {
		var key = arguments[1];
